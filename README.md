# AMS-AI

**晶圆厂设备告警管理系统** - 基于云原生架构的企业级告警解决方案，专为半导体制造行业设计。

## 📋 目录

- [技术栈](#技术栈)
- [项目结构](#项目结构)
- [功能特性](#功能特性)
- [快速开始](#快速开始)
- [API 文档](#api-文档)
- [架构设计](#架构设计)
- [开发规范](#开发规范)
- [部署指南](#部署指南)
- [路线图](#路线图)

---

## 技术栈

### 后端技术

| 组件 | 技术 | 版本 |
|------|------|------|
| 运行时框架 | Quarkus | 3.31.2 |
| 语言 | Java | 21 |
| 持久层 | Hibernate ORM + Panache Next | - |
| 数据库 | PostgreSQL / Oracle | 15+ / 19c+ |
| 数据迁移 | Liquibase | 4.27.0 |
| 分布式缓存 | Hazelcast | 5.4.0 |
| 安全认证 | SmallRye JWT | - |
| API 层 | REST + GraphQL (SmallRye) | - |
| AI 集成 | LangChain4j + Ollama | 0.29.1 |
| 监控 | Micrometer + Prometheus | 1.12.5 |

### 前端技术

| 组件 | 技术 | 版本 |
|------|------|------|
| 框架 | React | 18.2 |
| 语言 | TypeScript | 5.2 |
| 构建工具 | Vite | 5.0 |
| UI 组件 | Radix UI + Tailwind CSS | 4.x |
| 状态管理 | Zustand + TanStack Query | 4.4 / 5.90 |
| GraphQL 客户端 | graphql-request + json-bigint | 7.4 |
| 国际化 | i18next | 25.x |
| E2E 测试 | Playwright | 1.58 |

---

## 项目结构

```
ams-ai/
├── app-boot/                     # Quarkus 主程序入口
│   └── src/main/resources/
│       └── application.yml       # 主配置文件
│
├── app-web/                      # React 前端应用
│   ├── src/
│   │   ├── components/           # 通用组件
│   │   ├── features/             # 功能模块
│   │   ├── pages/                # 页面组件
│   │   ├── stores/               # Zustand 状态
│   │   ├── services/             # API 服务
│   │   └── i18n/                 # 国际化资源
│   └── e2e/                      # Playwright 测试
│
├── lib-common/                   # 公共模块
│   ├── exception/                # 异常定义
│   ├── dto/                      # 数据传输对象
│   └── constants/                # 常量定义
│
├── lib-persistence/               # 持久层
│   └── entity/                   # Hibernate 实体
│
├── lib-cluster/                   # 集群模块
│   └── Hazelcast 配置
│
├── feature-core/                  # 告警流水线核心
│   ├── 告警处理引擎
│   ├── 规则评估
│   └── 状态管理
│
├── feature-admin/                 # 管理后台
│   ├── 用户管理
│   ├── 角色管理
│   ├── 权限管理
│   └── 菜单管理
│
├── feature-graphql/               # GraphQL API
│   ├── entity/                    # 实体查询 API
│   │   ├── user/                  # 用户查询
│   │   ├── role/                  # 角色查询
│   │   ├── menu/                  # 菜单查询
│   │   ├── permission/            # 权限查询
│   │   ├── alarm/                 # 告警查询
│   │   └── dict/                  # 字典查询
│   ├── connection/                # 分页连接类型
│   └── filter/                    # 过滤条件转换
│
├── feature-alert-ingestion/       # 告警接入
│   ├── Webhook 接收
│   ├── API 接入
│   └── 消息队列集成
│
├── feature-ai-analysis/           # AI 智能分析
│   └── LangChain4j 集成
│
├── feature-notification/          # 通知渠道
│   ├── 邮件
│   ├── 短信
│   └── IM (钉钉/企业微信)
│
└── feature-multi-tenant/          # 多租户扩展
    └── 租户隔离
```

---

## 功能特性

### 核心功能 (P0)

#### 告警接入
- **多协议支持**: HTTP Webhook、gRPC、TCP Socket
- **消息队列集成**: Kafka/RabbitMQ/ActiveMQ
- **半导体行业特有**: EAP 集成、设备状态监控 (E10/E40/E90)
- **实时性能**: 消息处理延迟 < 100ms，支持 1000+ TPS

#### 告警处理引擎
- **告警持久化**: PostgreSQL/Oracle 存储
- **智能去重**: 基于指纹的告警去重
- **优先级计算**: 规则引擎 + AI 分析
- **状态流转**: 新建 → 确认 → 处理中 → 已解决 → 已关闭

#### 智能分析
- **AI 分类**: 基于内容的自动分类
- **根因分析**: 识别告警根本原因
- **趋势预测**: 预测告警趋势
- **螺旋告警检测**: 同一问题反复触发检测

#### 通知分发
- **多渠道**: 邮件、短信、钉钉、企业微信、Slack
- **通知策略**: 基于级别和时间的发送策略
- **告警升级**: 长时间未处理自动升级
- **模板管理**: 可配置的通知内容模板

#### 策略管理
- **规则引擎**: 基于条件的告警触发规则
- **静默规则**: 特定时间/条件下的告警静默
- **聚合规则**: 减少告警风暴
- **路由规则**: 告警自动路由到处理人

#### 多租户架构
- **数据隔离**: 按业务线隔离告警、配置、用户数据
- **配置隔离**: 每个业务线独立配置告警策略
- **权限隔离**: 用户只能访问所属业务线数据
- **策略插件化**: SPI 机制支持业务线定制策略

### 管理后台

| 模块 | 功能 |
|------|------|
| 告警大盘 | 实时列表、统计卡片、趋势图表、分布分析 |
| 告警管理 | 搜索筛选、详情查看、处理操作、评论关联 |
| 用户管理 | CRUD、角色分配、多业务线支持 |
| 角色管理 | CRUD、权限绑定、租户隔离 |
| 菜单管理 | 树形结构、权限控制、按钮级权限 |
| 数据字典 | 分类管理、树形字典项 |
| 业务线管理 | 租户创建、资源配额、策略插件 |

---

## 快速开始

### 环境要求

```bash
# 后端
JDK 21+
PostgreSQL 15+ 或 Oracle 19c+

# 前端
Node.js 18+
pnpm 9+
```

### 后端启动

```bash
# 克隆项目
git clone <repository-url>
cd ams-ai

# 开发模式 (热重载)
./gradlew quarkusDev

# 完整构建
./gradlew build

# 跳过测试构建
./gradlew build -x test

# 运行所有测试
./gradlew test

# 运行单个测试
./gradlew :app-boot:test --tests "pro.walkin.ams.boot.ApplicationTest"

# 数据库迁移
./gradlew liquibaseUpdate
```

### 前端启动

```bash
cd app-web

# 安装依赖
pnpm install

# 开发模式
pnpm dev

# 生产构建
pnpm build

# 代码检查
pnpm lint

# E2E 测试
pnpm test:e2e

# E2E 测试 (UI 模式)
pnpm test:e2e:ui
```

### 访问地址

| 服务 | 地址 |
|------|------|
| 前端应用 | http://localhost:5173 |
| 后端 API | http://localhost:8080 |
| Swagger UI | http://localhost:8080/q/swagger-ui/ |
| GraphQL UI | http://localhost:8080/q/graphql-ui/ |
| GraphQL API | http://localhost:8080/graphql |

---

## API 文档

### REST API

```bash
# 健康检查
GET /q/health

# 当前用户信息
GET /api/auth/me

# 当前用户菜单
GET /api/system/menus/user

# 当前用户权限
GET /api/system/permissions/user
```

### GraphQL 查询示例

```graphql
# 用户列表查询
query Users {
  users(page: 0, size: 10) {
    content {
      id
      username
      email
      enabled
      roles {
        id
        name
      }
    }
    totalElements
    totalPages
  }
}

# 角色带权限查询
query RoleWithPermissions($id: BigInteger!) {
  roles(where: { id: { eq: $id } }) {
    content {
      id
      name
      code
      permissions {
        id
        name
        code
      }
    }
  }
}

# 菜单树查询
query MenuTree {
  menus(where: { menuType: { eq: FOLDER } }) {
    content {
      id
      name
      code
      sortOrder
      children {
        id
        name
        path
      }
    }
  }
}
```

### API 架构

```
┌─────────────────────────────────────────────────────────┐
│                      前端应用                            │
├─────────────────────────────────────────────────────────┤
│  graphql-request  │  Axios + React Query                │
│  (查询)            │  (命令)                             │
└────────┬──────────┴───────────┬─────────────────────────┘
         │                      │
         ▼                      ▼
┌─────────────────┐    ┌─────────────────────────────────┐
│  /graphql       │    │  /api/*                         │
│  GraphQL API    │    │  REST API                       │
│  • 用户查询      │    │  • 创建/更新/删除               │
│  • 角色查询      │    │  • 当前用户信息                 │
│  • 菜单查询      │    │  • 认证相关                     │
│  • 权限查询      │    │                                 │
│  • 告警查询      │    │                                 │
└─────────────────┘    └─────────────────────────────────┘
```

---

## 架构设计

### 单体集群架构

```
                    ┌─────────────────────────────────────┐
                    │           负载均衡器                 │
                    └───────────────┬─────────────────────┘
                                    │
            ┌───────────────────────┼───────────────────────┐
            │                       │                       │
            ▼                       ▼                       ▼
    ┌───────────────┐       ┌───────────────┐       ┌───────────────┐
    │  Quarkus 实例  │       │  Quarkus 实例  │       │  Quarkus 实例  │
    │   (Node 1)    │       │   (Node 2)    │       │   (Node 3)    │
    └───────┬───────┘       └───────┬───────┘       └───────┬───────┘
            │                       │                       │
            └───────────────────────┼───────────────────────┘
                                    │
                    ┌───────────────┴───────────────┐
                    │        Hazelcast 集群          │
                    │   (分布式缓存 + 会话共享)       │
                    └───────────────┬───────────────┘
                                    │
                    ┌───────────────┴───────────────┐
                    │      PostgreSQL / Oracle      │
                    │        (主从复制)              │
                    └───────────────────────────────┘
```

### 数据模型

```sql
-- 核心表结构
users                 -- 用户表
├── id (PK)
├── tenant_id         -- 租户 ID
├── username
├── email
├── password_hash
└── enabled

roles                 -- 角色表
├── id (PK)
├── tenant_id
├── name
├── code
└── description

permissions           -- 权限表
├── id (PK)
├── name
├── code
├── type              -- MENU/BUTTON/API
└── menu_id (FK)

menus                 -- 菜单表
├── id (PK)
├── parent_id (FK)
├── name
├── code
├── path
├── menu_type         -- FOLDER/MENU/BUTTON
└── sort_order

alarms                -- 告警表
├── id (PK)
├── tenant_id
├── title
├── severity
├── status
├── source_id
├── fingerprint       -- 去重指纹
└── metadata (JSON)
```

### 安全架构

```
┌─────────────────────────────────────────────────────────┐
│                      JWT 认证流程                        │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  1. 用户登录 → 验证凭证 → 生成 JWT Token                 │
│  2. 请求携带 Token → 验证签名 → 提取用户信息             │
│  3. TenantContext → 获取当前租户 ID                      │
│  4. RBAC 检查 → 验证权限 → 允许/拒绝                     │
│                                                         │
└─────────────────────────────────────────────────────────┘

权限层级:
┌────────────────┐
│   全局权限      │  → 系统管理员
├────────────────┤
│   租户权限      │  → 业务线管理员
├────────────────┤
│   功能权限      │  → 普通用户
│   • 菜单权限    │
│   • 按钮权限    │
│   • API 权限    │
└────────────────┘
```

---

## 开发规范

### 代码风格

```java
// 实体类规范 (Panache Next)
@Entity
@Table(name = "alarms")
public class Alarm extends BaseEntity {
    
    @Column(name = "title")
    public String title;
    
    @JdbcTypeCode(SqlTypes.JSON)
    @Column(name = "metadata")
    public Map<String, Object> metadata;
    
    public interface Repo extends PanacheRepository<Alarm> {
        @Find
        Stream<Alarm> findBySourceId(String sourceId);
    }
}
```

```typescript
// 前端组件规范
import { useQuery } from '@tanstack/react-query'
import { graphqlClient } from '@/lib/graphqlClient'

export function useUsers(page: number, size: number) {
  return useQuery({
    queryKey: ['users', page, size],
    queryFn: () => graphqlClient.request(USERS_QUERY, { page, size }),
  })
}
```

### 提交规范

```
feat: 添加告警批量处理功能
fix: 修复角色删除时的 N+1 查询问题
docs: 更新 GraphQL API 文档
chore: 升级 Quarkus 到 3.31.2
refactor: 重构权限过滤逻辑
test: 添加角色管理 E2E 测试
```

### 质量检查

```bash
# SpotBugs 静态分析
./gradlew spotbugsMain

# 代码格式化
./gradlew spotlessApply

# 运行所有检查
./gradlew check

# 测试覆盖率
./gradlew jacocoTestReport
```

---

## 部署指南

### Docker 部署

```bash
# 构建镜像
./gradlew quarkusBuild -Dquarkus.container-image.build=true

# 运行容器
docker run -d \
  -p 8080:8080 \
  -e QUARKUS_DATASOURCE_JDBC_URL=jdbc:postgresql://db:5432/ams \
  -e QUARKUS_DATASOURCE_USERNAME=ams \
  -e QUARKUS_DATASOURCE_PASSWORD=secret \
  ams-ai:latest
```

### 原生镜像

```bash
# 构建原生可执行文件
./gradlew build -Dquarkus.native.enabled=true

# 运行原生镜像
./target/ams-ai-1.0.0-runner
```

### 配置文件

```yaml
# application.yml
quarkus:
  datasource:
    jdbc:
      url: jdbc:postgresql://localhost:5432/ams
    username: ams
    password: secret
  
  hibernate-orm:
    database:
      generation: none
  
  hazelcast-client:
    agents: localhost:5701
  
  smallrye-graphql:
    ui:
      enable: true  # 开发环境启用 GraphQL UI
```

---

## 路线图

### v1.0.0 (MVP)
- [x] 基础用户/角色/权限管理
- [x] GraphQL API 查询层
- [x] 前端管理界面
- [ ] 告警接入和处理
- [ ] 基础通知渠道

### v1.1.0
- [ ] 集群部署支持
- [ ] AI 分析能力
- [ ] 更多通知渠道
- [ ] 前端性能优化

### v1.2.0
- [ ] 高级策略引擎
- [ ] 可观测性增强
- [ ] 移动端优化
- [ ] 第三方集成

### v2.0.0
- [ ] 多租户完善
- [ ] 高级自动化
- [ ] 企业级安全
- [ ] 云原生部署

---

## 许可证

Private - All Rights Reserved

---

## 联系方式

如有问题，请联系开发团队或提交 Issue。
