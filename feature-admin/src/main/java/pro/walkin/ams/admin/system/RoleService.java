...
public class RoleService {
  ...
  public Role createRole(Role role) {
    ...
    if (existingRole.isPresent()
        && tenantId != null
        && tenantId.equals(existingRole.get().tenant)) {
  ...
  public void deleteRole(Long id) {
    long assignedUserCount =
        userRepo.count("select count(u) from User u join u.roles r where r.id = ?1", id);
  ...
  public boolean addPermissionToRole(Long roleId, Long permissionId) {
    ...
    if (role != null && permission != null) {
      ...
      boolean exists =
          role.permissions.stream().anyMatch(p -> p.id != null && p.id.equals(permissionId));
  ...
  public boolean assignUserToRole(Long roleId, Long userId) {
    ...
    if (role != null && user != null) {
      ...
      boolean exists = user.roles.stream().anyMatch(r -> r.id != null && r.id.equals(roleId));
