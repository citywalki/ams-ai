...
export function useMenuForm(options: UseMenuFormOptions) {
  ...
  const openCreateDialog = useCallback(() => {
    ...
    form.setFieldValue('menuType', 'MENU' as const);
    form.setFieldValue('rolesAllowed', '');
    setFormError(null);
    setDialogOpen(true);
  }, [form]);
  ...
  const openEditDialog = useCallback((menu: MenuItem) => {
    ...
    setEditingMenu(menu);
    form.reset();
    form.setFieldValue('key', menu.key);
    form.setFieldValue('label', menu.label);
    form.setFieldValue('route', menu.route || '');
    form.setFieldValue('icon', menu.icon || '');
    form.setFieldValue('sortOrder', menu.sortOrder);
    form.setFieldValue('isVisible', menu.isVisible);
    form.setFieldValue('menuType', menu.menuType);
    form.setFieldValue('rolesAllowed', (menu.rolesAllowed || []).join(','));
    setFormError(null);
    setDialogOpen(true);
  }, [form]);
