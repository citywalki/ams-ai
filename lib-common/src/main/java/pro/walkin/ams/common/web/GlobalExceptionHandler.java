 ...
 * 全局异常处理器
 *
 * <p>使用JAX-RS的ExceptionMapper处理全局异常，返回统一的错误响应。
 * 异常处理优先级：
 * 1. ValidationException（自定义验证异常）
 * 2. ConstraintViolationException（Bean Validation异常）
 * 3. NotFoundException（资源未找到）
 * 4. BusinessException（业务异常）
 * 5. BaseException（其他基础异常）
 * 6. WebApplicationException（JAX-RS异常）
    ...
    private Response.Status determineHttpStatus(String errorCode) {
        return switch (errorCode) {
            ...
            case Constants.ErrorCode.VALIDATION_ERROR, 
                 ...
                 Constants.ErrorCode.BAD_REQUEST -> Response.Status.BAD_REQUEST;
